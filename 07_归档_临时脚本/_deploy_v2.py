...
out, err = run(f"cp /home/ubuntu/config.py {PROJECT}/config.py && cp /home/ubuntu/client.py {PROJECT}/core/client.py && cp /home/ubuntu/pool.py {PROJECT}/core/pool.py && cp /home/ubuntu/db.py {PROJECT}/core/db.py && cp /home/ubuntu/main.py {PROJECT}/main.py && echo COPIED")
print(f"  Copy: {'OK' if 'COPIED' in out else 'FAILED ' + err[:100]}")
...
out, _ = run(f"grep 'reset_video_used' {PROJECT}/core/db.py | head -1")
print(f"  reset_video_used: {'YES' if out else 'NO'}")

out, _ = run(f"grep 'daily_claim_all' {PROJECT}/core/pool.py | head -1")
print(f"  daily_claim_all: {'YES' if out else 'NO'}")

out, _ = run(f"grep 'daily_claim_loop' {PROJECT}/main.py | head -1")
print(f"  daily_claim_loop: {'YES' if out else 'NO'}")
