...
# Serve frontend: pure ASGI middleware (BaseHTTPMiddleware has known issues with route matching)
class SPAMiddleware:
    """Pure ASGI SPA middleware - does NOT interfere with FastAPI route matching."""
    ...
    async def __call__(self, scope, receive, send):
        ...
        # Let API routes, docs, and OpenAPI be handled by FastAPI normally
        if path.startswith("/api/") or path.startswith("/admin/") or path.startswith("/docs") or path.startswith("/openapi") or path.startswith("/redoc"):
            await self.app(scope, receive, send)
            return

        # For other paths, try static file first, then SPA fallback
        fpath = os.path.join(FRONTEND_DIR, path.lstrip("/"))
        if os.path.isfile(fpath):
            from starlette.responses import FileResponse as StarletteFileResponse
            response = StarletteFileResponse(fpath)
        ...
        if os.path.exists(idx):
            from starlette.responses import FileResponse as StarletteFileResponse
            response = StarletteFileResponse(idx)
            await response(scope, receive, send)
            return
