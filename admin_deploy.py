...
app = FastAPI(title="AiForge Admin", version="1.0.0")
...
@app.post("/api/agents/{aid}/settle")
...
function ak(path) {
  ...
  return '/admin/api' + path + sep + 'key=' + KEY;
}
...
async function loadDashboard() {
  try {
    ...
    s.innerHTML = [
      ...
      {v: d.pool && d.pool.total_accounts || 0, l: '账号池', c: '#818cf8'},
...
async function toggleContinuousReg() {
  ...
  await fetch(ak('/pool/continuous_reg?enable=' + (action === 'start' ? 'true' : 'false') + '&target_count=50'), {method: 'POST'}).catch(function(){});
...
function copyText(text) {
  navigator.clipboard.writeText(text).then(function() { alert('已复制邀请链接: ' + text); }).catch(function() { prompt('请手动复制:', text); });
}
